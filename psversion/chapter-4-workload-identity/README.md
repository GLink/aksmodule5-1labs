...
. .\chapter-0-setup\lab-config.ps1
...
$STORAGE_SUFFIX = (Get-Date -UFormat %s).ToString()
$STORAGE_SUFFIX = $STORAGE_SUFFIX.Substring($STORAGE_SUFFIX.Length - 4)
$STORAGE_ACCOUNT = "stakslab${STUDENT_INITIALS}$STORAGE_SUFFIX"
...
Write-Host "✅ Granted Storage Blob Data Contributor role to current user"
...
# Upload a test file
$TEST_FILE = Join-Path $PWD "test-file.txt"
Set-Content -Path $TEST_FILE -Value "Hello from AKS Workload Identity!" -NoNewline
...
Write-Host "✅ Storage Blob Data Reader role assigned"
...
Write-Host "✅ Reader role assigned for storage account metadata"

# Wait for role propagation
Write-Host "⏳ Waiting 30 seconds for role assignments to propagate..."
Start-Sleep -Seconds 30
```
...
if (-not $TENANT_ID) {
  ...
  Write-Host "TENANT_ID set to: $TENANT_ID"
}
...
Write-Host "`n=== Verifying Federated Credential ==="
...
Write-Host "`n⏳ Waiting 30 seconds for federated credential to propagate..."
Start-Sleep -Seconds 30
```
...
kubectl logs blob-reader-pod -n workload-identity-demo | Select-String -Pattern "Hello from AKS"
```
...
Write-Host "✅ SQL identity created: $IDENTITY_SQL"
...
Write-Host "✅ SQL access service account and federated credential created"
```
...
Write-Host "✅ Deployment created with workload identity"
...
Write-Host "✅ Workload identity demo resources cleaned up"
```
